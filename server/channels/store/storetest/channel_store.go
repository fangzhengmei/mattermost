...
func TestChannelStore(t *testing.T, rctx request.CTX, ss store.Store, s SqlStore) {
	...
	t.Run("IsEncryptedPersistsOnInsert", func(t *testing.T) { testChannelStoreIsEncryptedPersistsOnInsert(t, rctx, ss) })
	t.Run("IsEncryptedUpdateFromTrueToFalseRejected", func(t *testing.T) { testChannelStoreIsEncryptedUpdateFromTrueToFalseRejected(t, rctx, ss) })
	t.Run("IsEncryptedUpdateFromFalseToTrueRejected", func(t *testing.T) { testChannelStoreIsEncryptedUpdateFromFalseToTrueRejected(t, rctx, ss) })
	t.Run("IsEncryptedUnchangedUpdateOK", func(t *testing.T) { testChannelStoreIsEncryptedUnchangedUpdateOK(t, rctx, ss) })
...
func testChannelStoreIsEncryptedPersistsOnInsert(t *testing.T, rctx request.CTX, ss store.Store) {
	teamID := model.NewId()

	encrypted := model.Channel{
		TeamId:      teamID,
	...
	saved, nErr := ss.Channel().Save(rctx, &encrypted, -1)
	require.NoError(t, nErr)
	require.True(t, saved.IsEncrypted)

	fetched, nErr := ss.Channel().Get(saved.Id, true)
	require.NoError(t, nErr)
	require.True(t, fetched.IsEncrypted, "IsEncrypted=true must round-trip through DB")
	...
	require.NoError(t, nErr)
	require.False(t, savedPlain.IsEncrypted)

	fetchedPlain, nErr := ss.Channel().Get(savedPlain.Id, true)
	require.NoError(t, nErr)
	require.False(t, fetchedPlain.IsEncrypted, "IsEncrypted=false must round-trip through DB")

	// Caller that doesn't know about IsEncrypted at all must still persist as false.
	defaulted := model.Channel{
		TeamId:      teamID,
		DisplayName: "Defaulted",
		Name:        NewTestID(),
		Type:        model.ChannelTypeOpen,
	}
	savedDefaulted, nErr := ss.Channel().Save(rctx, &defaulted, -1)
	require.NoError(t, nErr)
	require.False(t, savedDefaulted.IsEncrypted)

	fetchedDefaulted, nErr := ss.Channel().Get(savedDefaulted.Id, true)
	require.NoError(t, nErr)
	require.False(t, fetchedDefaulted.IsEncrypted, "omitted IsEncrypted must default to false")
}

func testChannelStoreIsEncryptedUpdateFromTrueToFalseRejected(t *testing.T, rctx request.CTX, ss store.Store) {
	channel := model.Channel{
		TeamId:      model.NewId(),
		DisplayName: "Encrypted",
		Name:        NewTestID(),
	...
	saved, nErr := ss.Channel().Save(rctx, &channel, -1)
	require.NoError(t, nErr)

	saved.IsEncrypted = false
	saved.DisplayName = "AttemptedToggleOff"
	_, err := ss.Channel().Update(rctx, saved)
	require.Error(t, err, "toggling IsEncrypted from true to false must be rejected")

	var invalidInputErr *store.ErrInvalidInput
	require.ErrorAs(t, err, &invalidInputErr, "error should be typed ErrInvalidInput")
	require.Equal(t, "IsEncrypted", invalidInputErr.Field)

	fetched, getErr := ss.Channel().Get(saved.Id, true)
	require.NoError(t, getErr)
	require.True(t, fetched.IsEncrypted, "DB row must remain IsEncrypted=true after rejected toggle")
}

func testChannelStoreIsEncryptedUpdateFromFalseToTrueRejected(t *testing.T, rctx request.CTX, ss store.Store) {
	channel := model.Channel{
		TeamId:      model.NewId(),
		DisplayName: "Plain",
		Name:        NewTestID(),
	...
	saved.IsEncrypted = true
	saved.DisplayName = "AttemptedToggleOn"
	_, err := ss.Channel().Update(rctx, saved)
	require.Error(t, err, "toggling IsEncrypted from false to true must be rejected")
	...
	fetched, getErr := ss.Channel().Get(saved.Id, true)
	require.NoError(t, getErr)
	require.False(t, fetched.IsEncrypted, "DB row must remain IsEncrypted=false after rejected toggle")
}

func testChannelStoreIsEncryptedUnchangedUpdateOK(t *testing.T, rctx request.CTX, ss store.Store) {
	channel := model.Channel{
		TeamId:      model.NewId(),
		DisplayName: "Encrypted",
		Name:        NewTestID(),
	...
	saved.DisplayName = "RenamedButStillEncrypted"
	saved.Header = "New header"
	updated, err := ss.Channel().Update(rctx, saved)
	require.NoError(t, err, "update with unchanged IsEncrypted must succeed")
	require.Equal(t, "RenamedButStillEncrypted", updated.DisplayName)
	require.Equal(t, "New header", updated.Header)
	require.True(t, updated.IsEncrypted, "IsEncrypted must remain true")

	fetched, getErr := ss.Channel().Get(saved.Id, true)
	require.NoError(t, getErr)
	require.Equal(t, "RenamedButStillEncrypted", fetched.DisplayName)
	require.True(t, fetched.IsEncrypted)
}
