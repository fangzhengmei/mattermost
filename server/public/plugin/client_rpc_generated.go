...
func init() {
	hookNameToId["CMECreateChannelKey"] = CMECreateChannelKeyID
}

type Z_CMECreateChannelKeyArgs struct {
	A string
}

type Z_CMECreateChannelKeyReturns struct {
	A *CMEHookResult
}

func (g *hooksRPCClient) CMECreateChannelKey(channelID string) *CMEHookResult {
	_args := &Z_CMECreateChannelKeyArgs{channelID}
	_returns := &Z_CMECreateChannelKeyReturns{}
	if g.implemented[CMECreateChannelKeyID] {
		if err := g.client.Call("Plugin.CMECreateChannelKey", _args, _returns); err != nil {
			g.log.Error("RPC call CMECreateChannelKey to plugin failed.", mlog.Err(err))
		}
	}
	return _returns.A
}

func (s *hooksRPCServer) CMECreateChannelKey(args *Z_CMECreateChannelKeyArgs, returns *Z_CMECreateChannelKeyReturns) error {
	if hook, ok := s.impl.(interface {
		CMECreateChannelKey(channelID string) *CMEHookResult
	}); ok {
		returns.A = hook.CMECreateChannelKey(args.A)
	} else {
		return encodableError(fmt.Errorf("Hook CMECreateChannelKey called but not implemented."))
	}
	return nil
}

func init() {
	hookNameToId["CMERevokeChannelKey"] = CMERevokeChannelKeyID
}

type Z_CMERevokeChannelKeyArgs struct {
	A string
}

type Z_CMERevokeChannelKeyReturns struct {
	A *CMEHookResult
}

func (g *hooksRPCClient) CMERevokeChannelKey(channelID string) *CMEHookResult {
	_args := &Z_CMERevokeChannelKeyArgs{channelID}
	_returns := &Z_CMERevokeChannelKeyReturns{}
	if g.implemented[CMERevokeChannelKeyID] {
		if err := g.client.Call("Plugin.CMERevokeChannelKey", _args, _returns); err != nil {
			g.log.Error("RPC call CMERevokeChannelKey to plugin failed.", mlog.Err(err))
		}
	}
	return _returns.A
}

func (s *hooksRPCServer) CMERevokeChannelKey(args *Z_CMERevokeChannelKeyArgs, returns *Z_CMERevokeChannelKeyReturns) error {
	if hook, ok := s.impl.(interface {
		CMERevokeChannelKey(channelID string) *CMEHookResult
	}); ok {
		returns.A = hook.CMERevokeChannelKey(args.A)
	} else {
		return encodableError(fmt.Errorf("Hook CMERevokeChannelKey called but not implemented."))
	}
	return nil
}

func init() {
	hookNameToId["CMEEncrypt"] = CMEEncryptID
}

type Z_CMEEncryptArgs struct {
	A string
	B []byte
}

type Z_CMEEncryptReturns struct {
	A *CMEEncryptResult
}

func (g *hooksRPCClient) CMEEncrypt(channelID string, plaintext []byte) *CMEEncryptResult {
	_args := &Z_CMEEncryptArgs{channelID, plaintext}
	_returns := &Z_CMEEncryptReturns{}
	if g.implemented[CMEEncryptID] {
		if err := g.client.Call("Plugin.CMEEncrypt", _args, _returns); err != nil {
			g.log.Error("RPC call CMEEncrypt to plugin failed.", mlog.Err(err))
		}
	}
	return _returns.A
}

func (s *hooksRPCServer) CMEEncrypt(args *Z_CMEEncryptArgs, returns *Z_CMEEncryptReturns) error {
	if hook, ok := s.impl.(interface {
		CMEEncrypt(channelID string, plaintext []byte) *CMEEncryptResult
	}); ok {
		returns.A = hook.CMEEncrypt(args.A, args.B)
	} else {
		return encodableError(fmt.Errorf("Hook CMEEncrypt called but not implemented."))
	}
	return nil
}

func init() {
	hookNameToId["CMEDecrypt"] = CMEDecryptID
}

type Z_CMEDecryptArgs struct {
	A string
	B [][]byte
}

type Z_CMEDecryptReturns struct {
	A *CMEDecryptBatchResult
}

func (g *hooksRPCClient) CMEDecrypt(channelID string, ciphertexts [][]byte) *CMEDecryptBatchResult {
	_args := &Z_CMEDecryptArgs{channelID, ciphertexts}
	_returns := &Z_CMEDecryptReturns{}
	if g.implemented[CMEDecryptID] {
		if err := g.client.Call("Plugin.CMEDecrypt", _args, _returns); err != nil {
			g.log.Error("RPC call CMEDecrypt to plugin failed.", mlog.Err(err))
		}
	}
	return _returns.A
}

func (s *hooksRPCServer) CMEDecrypt(args *Z_CMEDecryptArgs, returns *Z_CMEDecryptReturns) error {
	if hook, ok := s.impl.(interface {
		CMEDecrypt(channelID string, ciphertexts [][]byte) *CMEDecryptBatchResult
	}); ok {
		returns.A = hook.CMEDecrypt(args.A, args.B)
	} else {
		return encodableError(fmt.Errorf("Hook CMEDecrypt called but not implemented."))
	}
	return nil
}
